...
from src.objects.weapon import Weapon, Katana
...
#world class including all the rooms
class World:
    def __init__(self, wm, game, num_of_rooms, width, height):
        ...
        self.create_world()
    # Calls methods to generate rooms, assign types, add room maps, and assign objects.
    ...
    #Checks if a coordinate is within world boundaries.
    @staticmethod
    def check_boundary(coordinate, world_param): 
            ...
            for room in row:
                if isinstance(room, Room):
                    if room.type == 'chest':
                        ...
                        room.objects.append(Katana(self.game, room, (550, 300)))
        ...
        for row in self.world:
            for room in row:
                if isinstance(room, Room) and room.type is None:
                    room.type = random.choices(types, weights=[2, 4, 1], k=1)[0]
