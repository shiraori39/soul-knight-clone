...
#draw enemy health bar on top of it
...
#Base enemy class inherited from entity class
class Enemy(Entity):
    ...
    def move(self):
        if not self.dead and self.hp > 0 and self.can_move and not self.game.player.dead:
            if self.game.player.death_counter != 0:
                ...
                #go back to random state when player is dead
    ...
    #run a away from player - later used for ranged mob
